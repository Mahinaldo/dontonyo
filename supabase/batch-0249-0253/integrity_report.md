# Bounded Integrity Report — Jubayer’s GK Recovery Physical Pages 249–253

The idempotent recovery import transaction was executed against Supabase project `rennotctgrxvbpghbimx`. The bounded audit queried **only** physical source pages 249–253 and their five exact page-local provenance-topic slugs. The MCQ-option count is bounded through `public.gk_mcq_options o` joined to `public.gk_mcqs m ON m.id=o.mcq_id`.
