...
const work = "/home/ubuntu/dontonyo-work/batch-0684-0688";
const out = path.join(root, "supabase", "batch-0684-0688");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [684, 685, 686, 687, 688];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0684-0688-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0684-0688-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 684 AND 688";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 684–688 are included.", "MCQs require four visually reconciled options, a printed key, readable source label, and corroborated safe premise.", "Only allowed tag categories are inserted.", "Political, colonial, territorial, religious, security-sensitive, ideological, and source-error claims are withheld rather than imported."]
  };
...
FROM public.books b WHERE b.title=${q(bookTitle)} AND NOT EXISTS (SELECT 1 FROM public.source_pages s WHERE s.book_id=b.id AND s.source_page=${page.page});`;
  ...
  const sql = `-- Generated by prepare_validated_batch_0684_0688.mjs. Source pages 684–688 only.\nBEGIN;\nINSERT INTO public.import_runs (source_filename,source_sha256,pipeline_version,status,completed_at,audit) VALUES ('Jubayer''sgk.pdf',${q(hash(pages.map(page => page.sha256).join("|")))},${q(PIPELINE_VERSION)},'completed',now(),${j(audit)});\n${topicSql}\n${pageSql}\n${sqlForFacts(facts)}\n${sqlForNotes(notes)}\n${sqlForMcqs(mcqs)}\n${sqlForTags(records)}\n${sqlForVerifications(records)}\n${sqlForDerived()}\nCOMMIT;`;
  return { sql, audit, counts: { batch_pages: BATCH_PAGES, review_tiles: 35, eligible_facts: facts.length, eligible_notes: notes.length, eligible_mcqs: mcqs.length, eligible_mcq_options: mcqs.length * 4, withheld_mcqs: 10, derived_records: records.length, verification_statuses: audit.verification_counts } };
}
