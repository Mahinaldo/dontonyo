...
const work = "/home/ubuntu/dontonyo-work/batch-0619-0623";
const out = path.join(root, "supabase", "batch-0619-0623");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [619, 620, 621, 622, 623];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0619-0623-v1";
...
async function load() {
  ...
  const pages = await Promise.all(BATCH_PAGES.map(async page => {
    ...
    const raw = await fs.readFile(path.join(work, "ocr", `page_${String(page).padStart(4, "0")}_upright_psm6.txt`), "utf8");
    return { page, sha256: hash(image), raw };
  }));
  const facts = content.facts.map(item => ({ ...item, type: "fact", hash: hash(`fact|${item.page}|${item.title}|${item.text}`), confidence: item.status === "verified" ? "high" : "medium" }));
  const notes = content.notes.map(item => ({ ...item, type: "note", hash: hash(`note|${item.page}|${item.title}|${item.text}`), confidence: "high" }));
  const mcqs = content.mcqs.map(item => ({ ...item, topic: content.topics.find(topic => topic.page === item.page)?.slug, type: "mcq", hash: hash(`mcq|${item.page}|${item.number}|${item.question}`), confidence: item.status === "verified" ? "high" : "medium" }));
...
function sqlForTags(records) {
  const tags = [
    ...
    ["reference-note", "Structured source note", "content_type", "Bounded source reference with caveats."], ["past-exam-mcq", "Past-exam MCQ", "content_type", "Complete MCQ with a visually reviewed printed answer key."], ["answer-key", "Printed answer key", "content_type", "Visually reviewed source answer key."],
  ...
  const assignments = records.map(item => {
    const shared = ["africa", item.domain, verificationTag(item), "historic-scope"];
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0619-0623-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 619 AND 623";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: ["All 35 ordered overlap-safe source tiles were reviewed.", "The rendered source images were already upright; clockwise copies were visually sideways and excluded from OCR provenance.", "Source errors, current claims, unsupported epithets, rankings, and unsafe MCQ premises were withheld rather than reconstructed."],
    quality_gates: ["Only source pages 619–623 are included.", "Every imported MCQ has four source-visible options and exactly one visually confirmed printed key.", "Only allowed tag categories are inserted.", "Political and institutional records use neutral historic scope; no source-error MCQ is imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_619_623.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image verified upright; clockwise copy was excluded after visual inspection." };
...
FROM public.books b WHERE b.title=${q(bookTitle)} AND NOT EXISTS (SELECT 1 FROM public.source_pages s WHERE s.book_id=b.id AND s.source_page=${page.page});`;
  ...
  const sql = `-- Generated by prepare_validated_batch_0619_0623.mjs. Source pages 619–623 only.\nBEGIN;\nINSERT INTO public.import_runs (source_filename,source_sha256,pipeline_version,status,completed_at,audit) VALUES ('Jubayer''sgk.pdf',${q(hash(pages.map(page => page.sha256).join("|")))},${q(PIPELINE_VERSION)},'completed',now(),${j(audit)});\n${topicSql}\n${pageSql}\n${sqlForFacts(facts)}\n${sqlForNotes(notes)}\n${sqlForMcqs(mcqs)}\n${sqlForTags(records)}\n${sqlForVerifications(records)}\n${sqlForDerived()}\nCOMMIT;`;
  return { sql, audit, counts: { batch_pages: BATCH_PAGES, review_tiles: 35, eligible_facts: facts.length, eligible_notes: notes.length, eligible_mcqs: mcqs.length, eligible_mcq_options: mcqs.length * 4, withheld_mcqs: 6, derived_records: records.length, verification_statuses: audit.verification_counts } };
}
