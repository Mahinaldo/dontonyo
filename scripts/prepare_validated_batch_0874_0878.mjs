...
const work = "/home/ubuntu/dontonyo-work/batch-0874-0878";
const out = path.join(root, "supabase", "batch-0874-0878");
const quote = value => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = value => `${quote(JSON.stringify(value))}::jsonb`;
const hash = value => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [874, 875, 876, 877, 878];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0874-0878-v1";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: [
      ...
      "OCR degradation, table omissions, and source-imprecise vocabulary mappings were preserved rather than normalized.",
      "Financial, media, institutional, political/security, religious, military, national, identity-sensitive, territorial, geographic, historical, ranking, current-status, and date-sensitive claims were withheld.",
    ],
    quality_gates: [
      "Only source pages 874–878 are included.",
  ...
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${out}/visual_review_874_878.md`, classification_report: `${out}/classification_decisions.md`, verification_report: `${out}/external_verification.md`, orientation_note: "Rendered source image visually verified upright before local OCR." };
