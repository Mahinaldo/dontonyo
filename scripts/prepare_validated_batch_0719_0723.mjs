...
const work = "/home/ubuntu/dontonyo-work/batch-0719-0723";
const out = path.join(root, "supabase", "batch-0719-0723");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [719, 720, 721, 722, 723];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0719-0723-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0719-0723-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 719 AND 723";
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${out}/visual_review_719_723.md`, classification_report: `${out}/classification_decisions.md`, verification_report: `${out}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
