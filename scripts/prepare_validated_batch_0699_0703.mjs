...
const work = "/home/ubuntu/dontonyo-work/batch-0699-0703";
const out = path.join(root, "supabase", "batch-0699-0703");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [699, 700, 701, 702, 703];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0699-0703-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0699-0703-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 699 AND 703";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: ["All 35 ordered overlap-safe source tiles were reviewed.", "The rendered source images were already upright and upright local OCR was reconciled with visual evidence.", "Security-sensitive, current, territorial, political, and dynamically changing source claims were withheld rather than reconstructed."],
    quality_gates: ["Only source pages 699–703 are included.", "MCQs require four visually reconciled options, a printed key, readable source label, and corroborated safe premise.", "Only allowed tag categories are inserted.", "Current, technical, national-status, political, multi-keyed, starred, and source-error claims are withheld rather than imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_699_703.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
...
FROM public.books b WHERE b.title=${q(bookTitle)} AND NOT EXISTS (SELECT 1 FROM public.source_pages s WHERE s.book_id=b.id AND s.source_page=${page.page});`;
  ...
  return { sql, audit, counts: { batch_pages: BATCH_PAGES, review_tiles: 35, eligible_facts: facts.length, eligible_notes: notes.length, eligible_mcqs: mcqs.length, eligible_mcq_options: mcqs.length * 4, withheld_mcqs: 14, derived_records: records.length, verification_statuses: audit.verification_counts } };
}
