...
const work = "/home/ubuntu/dontonyo-work/batch-0664-0668";
const out = path.join(root, "supabase", "batch-0664-0668");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [664, 665, 666, 667, 668];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0664-0668-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0664-0668-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 664 AND 668";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 664–668 are included.", "No MCQ is included because the reviewed source range has no printed answer-keyed question block.", "Only allowed tag categories are inserted.", "Political labels and ancient-history superlatives use neutral historic scope; no source-error claim is imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_664_668.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
