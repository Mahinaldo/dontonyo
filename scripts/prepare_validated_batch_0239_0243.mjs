...
const work = "/home/ubuntu/dontonyo-work/batch-0239-0243";
const out = path.join(root, "supabase", "batch-0239-0243");
const quote = value => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = value => `${quote(JSON.stringify(value))}::jsonb`;
const hash = value => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [239, 240, 241, 242, 243];
export const PIPELINE_VERSION = "local-ocr-quality-gated-recovery-batch-0239-0243-v1";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: ["All 35 ordered overlap-safe recovery tiles were reviewed.", "Local Bangla-English OCR was reconciled with visual evidence; image evidence controlled.", "OCR degradation and visually uncertain spellings were preserved without silent correction.", "Political, military/security, diplomatic, international-relations, national, historical, cultural/literary-attribution, media, institutional, source-corrected, named-person, and date-sensitive claims were withheld."],
    quality_gates: ["Only recovery source pages 239–243 are included.", "No reviewed claim clears the conservative quality, policy-safety, internal-consistency, and authority-verification boundary.", "Only reviewed source provenance and page-local topic boundaries are admitted."]
  };
