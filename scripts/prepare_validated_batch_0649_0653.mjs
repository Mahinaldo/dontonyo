...
const work = "/home/ubuntu/dontonyo-work/batch-0649-0653";
const out = path.join(root, "supabase", "batch-0649-0653");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [649, 650, 651, 652, 653];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0649-0653-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0649-0653-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 649 AND 653";
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_649_653.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
...
FROM public.books b WHERE b.title=${q(bookTitle)} AND NOT EXISTS (SELECT 1 FROM public.source_pages s WHERE s.book_id=b.id AND s.source_page=${page.page});`;
  ...
  const sql = `-- Generated by prepare_validated_batch_0649_0653.mjs. Source pages 649–653 only.\nBEGIN;\nINSERT INTO public.import_runs (source_filename,source_sha256,pipeline_version,status,completed_at,audit) VALUES ('Jubayer''sgk.pdf',${q(hash(pages.map(page => page.sha256).join("|")))},${q(PIPELINE_VERSION)},'completed',now(),${j(audit)});\n${topicSql}\n${pageSql}\n${sqlForFacts(facts)}\n${sqlForNotes(notes)}\n${sqlForMcqs(mcqs)}\n${sqlForTags(records)}\n${sqlForVerifications(records)}\n${sqlForDerived()}\nCOMMIT;`;
  return { sql, audit, counts: { batch_pages: BATCH_PAGES, review_tiles: 35, eligible_facts: facts.length, eligible_notes: notes.length, eligible_mcqs: mcqs.length, eligible_mcq_options: mcqs.length * 4, withheld_mcqs: 5, derived_records: records.length, verification_statuses: audit.verification_counts } };
}
