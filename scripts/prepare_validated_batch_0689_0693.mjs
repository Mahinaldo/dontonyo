...
const work = "/home/ubuntu/dontonyo-work/batch-0689-0693";
const out = path.join(root, "supabase", "batch-0689-0693");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [689, 690, 691, 692, 693];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0689-0693-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0689-0693-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 689 AND 693";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: ["All 35 ordered overlap-safe source tiles were reviewed.", "The rendered source images were already upright and upright local OCR was reconciled with visual evidence.", "Security-sensitive arms-control, nuclear-policy, territorial, diplomatic, and politically compressed source claims were withheld rather than reconstructed."],
    quality_gates: ["Only source pages 689–693 are included.", "No MCQ is included because no complete past-exam MCQ with four options and a visually verified printed key appears in the source pages.", "Only allowed tag categories are inserted.", "Only a neutral ICRC-verified Geneva Conventions record is admitted; security-sensitive and politically sensitive source claims are preserved only as boundary notes."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_689_693.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
