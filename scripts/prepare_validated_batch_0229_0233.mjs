...
const work = "/home/ubuntu/dontonyo-work/batch-0229-0233";
const out = path.join(root, "supabase", "batch-0229-0233");
const quote = value => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = value => `${quote(JSON.stringify(value))}::jsonb`;
const hash = value => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [229, 230, 231, 232, 233];
export const PIPELINE_VERSION = "local-ocr-quality-gated-recovery-batch-0229-0233-v1";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: [
      ...
      "Military-security, political, national, historical, atrocity/death, medical, institutional, recognition, biographical, location, numerical, source-corrected/disputed, attribution, cultural/sport, and date-sensitive claims were withheld."
    ],
    quality_gates: [
      "Only recovery source pages 229–233 are included.",
  ...
  const tags = [
    ...
    ["military-biography-reference", "Military biography reference", "domain", "Military biography material retained as reviewed source provenance."],
    ["institutional-history-reference", "Institutional history reference", "domain", "Institutional-history material retained as reviewed source provenance."],
