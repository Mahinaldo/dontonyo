...
const work = "/home/ubuntu/dontonyo-work/batch-0654-0658";
const out = path.join(root, "supabase", "batch-0654-0658");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [654, 655, 656, 657, 658];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0654-0658-v1";
...
function sqlForTags(records) {
  const tags = [
    ["oceania", "Oceania", "domain", "Oceania source material."], ["africa", "Africa", "domain", "Africa source material."], ["world-regions", "World regions", "domain", "Regional geography source material."], ["world-history", "World history", "domain", "Historically scoped world-reference material."],
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0654-0658-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 654 AND 658";
...
SELECT (SELECT id FROM public.subjects ORDER BY created_at ASC LIMIT 1),f.book_id,f.chapter_id,f.topic_id,f.title,f.fact_text,'fact',f.id,'batch0654-0658:fact:'||f.id::text FROM public.gk_facts f WHERE f.source_page ${range} AND NOT EXISTS (SELECT 1 FROM public.flashcards z WHERE z.source_key='batch0654-0658:fact:'||f.id::text);
...
SELECT (SELECT id FROM public.subjects ORDER BY created_at ASC LIMIT 1),n.book_id,n.chapter_id,n.topic_id,n.title,n.content,'note',n.id,'batch0654-0658:note:'||n.id::text FROM public.gk_notes n WHERE n.source_page ${range} AND NOT EXISTS (SELECT 1 FROM public.flashcards z WHERE z.source_key='batch0654-0658:note:'||n.id::text);
INSERT INTO public.flashcards (subject_id,book_id,chapter_id,topic_id,front_text,back_text,source_type,source_id,source_key)
SELECT (SELECT id FROM public.subjects ORDER BY created_at ASC LIMIT 1),m.book_id,m.chapter_id,m.topic_id,m.question,'সঠিক উত্তর: '||o.option_key||'. '||o.option_text,'mcq',m.id,'batch0654-0658:mcq:'||m.id::text FROM public.gk_mcqs m JOIN public.gk_mcq_options o ON o.mcq_id=m.id AND o.is_correct WHERE m.source_page ${range} AND NOT EXISTS (SELECT 1 FROM public.flashcards z WHERE z.source_key='batch0654-0658:mcq:'||m.id::text);
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 654–658 are included.", "MCQs require four visually reconciled options, a printed key, readable source label, and corroborated safe premise.", "Only allowed tag categories are inserted.", "Security-sensitive, territorial, and source-error claims are withheld rather than imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_654_658.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
...
FROM public.books b WHERE b.title=${q(bookTitle)} AND NOT EXISTS (SELECT 1 FROM public.source_pages s WHERE s.book_id=b.id AND s.source_page=${page.page});`;
  ...
  const sql = `-- Generated by prepare_validated_batch_0654_0658.mjs. Source pages 654–658 only.\nBEGIN;\nINSERT INTO public.import_runs (source_filename,source_sha256,pipeline_version,status,completed_at,audit) VALUES ('Jubayer''sgk.pdf',${q(hash(pages.map(page => page.sha256).join("|")))},${q(PIPELINE_VERSION)},'completed',now(),${j(audit)});\n${topicSql}\n${pageSql}\n${sqlForFacts(facts)}\n${sqlForNotes(notes)}\n${sqlForMcqs(mcqs)}\n${sqlForTags(records)}\n${sqlForVerifications(records)}\n${sqlForDerived()}\nCOMMIT;`;
  return { sql, audit, counts: { batch_pages: BATCH_PAGES, review_tiles: 35, eligible_facts: facts.length, eligible_notes: notes.length, eligible_mcqs: mcqs.length, eligible_mcq_options: mcqs.length * 4, withheld_mcqs: 12, derived_records: records.length, verification_statuses: audit.verification_counts } };
}
