...
const work = "/home/ubuntu/dontonyo-work/batch-0864-0868";
const out = path.join(root, "supabase", "batch-0864-0868");
const quote = value => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = value => `${quote(JSON.stringify(value))}::jsonb`;
const hash = value => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [864, 865, 866, 867, 868];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0864-0868-v1";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: [
      ...
      "OCR degradation and an unresolved page-866 name discrepancy were preserved rather than normalized.",
      "Religious, political, national, gender, identity-sensitive, award/ranking, institutional, monetary/current-status, historical, geographic, biographical, and date-sensitive claims were withheld.",
    ],
    quality_gates: [
      "Only source pages 864–868 are included.",
  ...
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${out}/visual_review_864_868.md`, classification_report: `${out}/classification_decisions.md`, verification_report: `${out}/external_verification.md`, orientation_note: "Rendered source image visually verified upright before local OCR." };
