...
const work = "/home/ubuntu/dontonyo-work/batch-0634-0638";
const out = path.join(root, "supabase", "batch-0634-0638");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [634, 635, 636, 637, 638];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0634-0638-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0634-0638-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 634 AND 638";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 634–638 are included.", "No MCQ is included because the reviewed source pages contain no printed answer-keyed question block.", "Only allowed tag categories are inserted.", "Political and institutional records use neutral historic scope; no source-error claim is imported."]
  };
