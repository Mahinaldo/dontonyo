...
const work = "/home/ubuntu/dontonyo-work/batch-0694-0698";
const out = path.join(root, "supabase", "batch-0694-0698");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [694, 695, 696, 697, 698];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0694-0698-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0694-0698-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 694 AND 698";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 694–698 are included.", "MCQs require four visually reconciled options, a printed key, readable source label, and corroborated safe premise.", "Only allowed tag categories are inserted.", "Security-sensitive, current, territorial, political, and source-error claims are withheld rather than imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_694_698.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
