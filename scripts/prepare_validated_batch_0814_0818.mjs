...
const work = "/home/ubuntu/dontonyo-work/batch-0814-0818";
const out = path.join(root, "supabase", "batch-0814-0818");
const quote = (value) => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = (value) => `${quote(JSON.stringify(value))}::jsonb`;
const hash = (value) => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [814, 815, 816, 817, 818];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0814-0818-v1";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    source_anomalies: [
      ...
      "Geographic, territorial, country-status, location-linked, seaport-list, river/canal, ranking/superlative, and source-imprecise claims were withheld."
    ],
    quality_gates: [
      "Only source pages 814–818 are included.",
  ...
  const pageSql = pages.map((page) => {
    ...
    const metadata = {
      ...
      review_report: `${out}/visual_review_814_818.md`,
  ...
  const sql = `-- Generated by prepare_validated_batch_0814_0818.mjs. Source pages 814–818 only.
BEGIN;
