...
const out = path.join(root, "supabase", "batch-0629-0633");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [629, 630, 631, 632, 633];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0629-0633-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0629-0633-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 629 AND 633";
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 629–633 are included.", "No MCQ is included because the reviewed source pages contain no printed answer-keyed question block.", "Only allowed tag categories are inserted.", "Political and institutional records use neutral historic scope; no source-error claim is imported."]
  };
...
AND NOT EXISTS (SELECT 1 FROM public.topics t WHERE t.chapter_id=c.id AND t.slug=${q(topic.slug)});`).join("\n");
  const pageSql = pages.map(page => {
    ...
    const metadata = { physical_source_page: page.page, source_image_sha256: page.sha256, review_status: "completed_image_grounded_review", review_report: `${work}/visual_review_629_633.md`, classification_report: `${work}/classification_decisions.md`, verification_report: `${work}/external_verification.md`, recovery_artifacts: [`page_${String(page.page).padStart(4, "0")}_upright_psm6.txt`], orientation_note: "Rendered source image visually verified upright before local OCR." };
