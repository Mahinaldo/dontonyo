...
const work = "/home/ubuntu/dontonyo-work/batch-0269-0273";
const out = path.join(root, "supabase", "batch-0269-0273");
const quote = value => `'${String(value ?? "").replaceAll("'", "''")}'`;
const json = value => `${quote(JSON.stringify(value))}::jsonb`;
const hash = value => createHash("sha256").update(value).digest("hex");

export const BATCH_PAGES = [269, 270, 271, 272, 273];
export const PIPELINE_VERSION = "local-ocr-quality-gated-recovery-batch-0269-0273-v1";

export async function buildBatch() {
  const content = JSON.parse(await fs.readFile(path.join(out, "approved_content.json"), "utf8"));
  const pages = await Promise.all(BATCH_PAGES.map(async page => ({ page, sha256: hash(await fs.readFile(path.join(work, "rendered", `page-${page}.jpg`))), raw: await fs.readFile(path.join(work, "ocr", `page-0${page}.txt`), "utf8") })));
  const audit = {
    batch_pages: BATCH_PAGES, pipeline_version: PIPELINE_VERSION,
    generated_facts: 0, generated_notes: 0, generated_mcqs: 0, generated_options: 0, generated_flashcards: 0,
    ...
    source_anomalies: ["All 35 ordered overlap-safe recovery tiles were reviewed.","Local Bangla-English OCR was reconciled with visual evidence; image evidence controlled.","OCR degradation, printed answer keys, source notation, dates, named people, and captions were preserved without silent correction.","National, constitutional, political, government, legal, historical, cultural, military, international, memorial/death, source-answer-keyed, source-noted, design/measurement, named-person/place, and date-sensitive claims were withheld."],
    quality_gates: ["Only recovery source pages 269–273 are included.","No reviewed claim clears the conservative quality, policy-safety, internal-consistency, answer-key reliability, and authority-verification boundary.","Only reviewed source provenance and page-local topic boundaries are admitted."],
  };
