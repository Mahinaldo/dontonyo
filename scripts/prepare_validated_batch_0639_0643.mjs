...
const work = "/home/ubuntu/dontonyo-work/batch-0639-0643";
const out = path.join(root, "supabase", "batch-0639-0643");
const bookTitle = "Jubayer's GK";
export const BATCH_PAGES = [639, 640, 641, 642, 643];
export const PIPELINE_VERSION = "local-ocr-quality-gated-batch-0639-0643-v1";
...
SELECT t.id,${q(item.type)},${entity(item.type, item)},${item.page},${q(item.confidence)}::confidence_level,'batch-0639-0643-quality-pipeline'
...
function sqlForDerived() {
  const range = "BETWEEN 639 AND 643";
...
SELECT (SELECT id FROM public.subjects ORDER BY created_at ASC LIMIT 1),f.book_id,f.chapter_id,f.topic_id,f.title,f.fact_text,'fact',f.id,'batch0639-0643:fact:'||f.id::text FROM public.gk_facts f WHERE f.source_page ${range} AND NOT EXISTS (SELECT 1 FROM public.flashcards z WHERE z.source_key='batch0639-0643:fact:'||f.id::text);
...
SELECT (SELECT id FROM public.subjects ORDER BY created_at ASC LIMIT 1),n.book_id,n.chapter_id,n.topic_id,n.title,n.content,'note',n.id,'batch0639-0643:note:'||n.id::text FROM public.gk_notes n WHERE n.source_page ${range} AND NOT EXISTS (SELECT 1 FROM public.flashcards z WHERE z.source_key='batch0639-0643:note:'||n.id::text);
INSERT INTO public.flashcards (subject_id,book_id,chapter_id,topic_id,front_text,back_text,source_type,source_id,source_key)
...
export async function buildBatch() {
  ...
  const audit = {
    ...
    quality_gates: ["Only source pages 639–643 are included.", "No MCQ is included because final item-by-item answer-key reconciliation is intentionally incomplete.", "Only allowed tag categories are inserted.", "Political and institutional records use neutral historic scope; no source-error claim is imported."]
  };
