...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0224_0228.mjs";

describe("validated recovery pages 224–228 provenance-only import contract", () => {
  it("locks recovery range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([224, 225, 226, 227, 228]);
  ...
  it("retains transaction, idempotency, and no learner-content inserts", async () => {
    ...
    expect(sql).toContain("-- Generated by prepare_validated_batch_0224_0228.mjs. Recovery source pages 224–228 only.");
