...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0824_0828.mjs";

describe("validated pages 824–828 provenance-only import contract", () => {
  it("locks the reviewed physical range and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([824, 825, 826, 827, 828]);
  ...
  it("retains only source-page boundaries with transaction and import-run idempotency", async () => {
    ...
    expect(sql).toContain("batch-0824-0828");
