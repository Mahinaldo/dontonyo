...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0849_0853.mjs";

describe("validated pages 849–853 provenance-only import contract", () => {
  it("locks range and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([849, 850, 851, 852, 853]);
