...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0234_0238.mjs";

describe("validated recovery pages 234–238 provenance-only import contract", () => {
  it("locks recovery range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([234, 235, 236, 237, 238]);
  ...
  it("retains transaction, idempotency, and no learner-content inserts", async () => {
    ...
    expect(sql).toContain("-- Generated by prepare_validated_batch_0234_0238.mjs. Recovery source pages 234–238 only.");
