...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0239_0243.mjs";

describe("validated recovery pages 239–243 provenance-only import contract", () => {
  it("locks recovery range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([239, 240, 241, 242, 243]);
  ...
  it("retains transaction, idempotency, and no learner-content inserts", async () => {
    ...
    expect(sql).toContain("-- Generated by prepare_validated_batch_0239_0243.mjs. Recovery source pages 239–243 only.");
