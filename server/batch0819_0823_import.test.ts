...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0819_0823.mjs";

describe("validated pages 819–823 provenance-only import contract", () => {
  it("locks the reviewed physical range and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([819, 820, 821, 822, 823]);
  ...
  it("retains only source-page boundaries with transaction and import-run idempotency", async () => {
    ...
    expect(sql).toContain("batch-0819-0823");
