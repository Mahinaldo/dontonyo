...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0899_0903.mjs";

describe("validated pages 899–903 provenance-only import contract", () => {
  it("locks range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([899, 900, 901, 902, 903]);
  ...
  it("retains transaction, idempotency, and no learner-content inserts", async () => {
    ...
    expect(sql).toContain("-- Generated by prepare_validated_batch_0899_0903.mjs. Source pages 899–903 only.");
