...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0209_0213.mjs";

describe("validated recovery pages 209–213 provenance-only import contract", () => {
  it("locks recovery range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([209, 210, 211, 212, 213]);
  ...
  it("retains transaction, idempotency, and no learner-content inserts", async () => {
    ...
    expect(sql).toContain("-- Generated by prepare_validated_batch_0209_0213.mjs. Recovery source pages 209–213 only.");
