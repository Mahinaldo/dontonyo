...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0889_0893.mjs";

describe("validated pages 889–893 provenance-only import contract", () => {
  it("locks range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([889, 890, 891, 892, 893]);
