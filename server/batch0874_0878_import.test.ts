...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0874_0878.mjs";

describe("validated pages 874–878 provenance-only import contract", () => {
  it("locks range, tile review, and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([874, 875, 876, 877, 878]);
