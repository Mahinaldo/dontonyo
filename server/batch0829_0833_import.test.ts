...
import { BATCH_PAGES, PIPELINE_VERSION, buildBatch } from "../scripts/prepare_validated_batch_0829_0833.mjs";

describe("validated pages 829–833 provenance-only import contract", () => {
  it("locks the reviewed physical range and zero-admission totals", async () => {
    const { audit, counts } = await buildBatch();
    expect(BATCH_PAGES).toEqual([829, 830, 831, 832, 833]);
  ...
  it("retains only source-page boundaries with transaction and import-run idempotency", async () => {
    ...
    expect(sql).toContain("batch-0829-0833");
